...
- [ ] **KasirPro** — dashboard POS bespoke (sudah bukan template marketing). Cek polish/konsistensi dgn standar baru. Demo terakhir.
- ✅ **6 dari 7 demo marketing sudah premium-rebuild** (Amara, Safar, Sajira, TripNusa, Kopi, PrimaLand, EximTrade). `DemoPage`/`demos.ts` kini hanya dipakai... tidak ada lagi (semua page sudah pakai komponen bespoke). Bisa pertimbangkan hapus `DemoPage.tsx` + `demos.ts` kalau tak terpakai (cek import dulu).
