...
- [x] **EximTrade** (`EximTradeDemo.tsx`) — B2B ekspor-impor, **English copy**, slate #2c4a63 + amber #e3a547. Layout korporat distinct: nav transparan + "Request Quote", hero pelabuhan kontainer, **credentials bar** (45+ Countries/Incoterms/ISO-HACCP/Shipping), **commodity catalog** (5 kartu B2B: grade/MOQ/packaging/origin + 1 tile CTA "View Full Catalog"), about split (cargo ship) + stats, **export process band slate** (4 langkah: Inquiry→Quotation→Sourcing&QC→Shipping), global-reach region chips, CTA "Request a Quote". Gambar diperbaiki: 2 komoditas SALAH (kentang dilabel "Cacao", mangga dilabel "Rattan") → cacao pods `1573710661345` + frozen shrimp `1504309250229` (baru, terverifikasi); mangga di-relabel jadi "Tropical Fruits" (akurat); + cargo ship `1679183959103`. Rattan di-drop (tak ada foto free terverifikasi) → diganti tile CTA. commit <THIS>
- [x] **KasirPro** (`KasirProDemo.tsx`) — dashboard POS di-rebuild premium (client component): sidebar dgn active-bar + "Buka Kasir" + user card, topbar + "Transaksi Baru", 4 KPI (trend pill), **chart 7-hari** (hover value, Sabtu di-highlight), **breakdown metode pembayaran** (QRIS/Tunai/Kartu/E-wallet progress), tabel transaksi (kolom metode + status), produk terlaris (rank+progress), **alert stok menipis**, reveal animasi (Rise). Fix bug `h-4.5/w-4.5` (size Tailwind invalid → ikon 0px) jadi h-4/h-5. commit <THIS>
- [x] **Cleanup**: hapus `src/components/demo/DemoPage.tsx` + `src/data/demos.ts` (dead code — semua 8 demo kini komponen bespoke). Build SSG 8 demo lolos.

> ✅ **Tahap 8.5 SELESAI** — 8 demo (7 marketing + KasirPro) semua premium & gambar terverifikasi. Siap fokus ke perbaikan/tahap lain.
...
- 2026-06-26 (Hero redesign): Ganti hero 3D R3F lama (blob distort "jelek") → **Ember Phoenix** (`src/components/hero/EmberPhoenix.tsx`): canvas particle yang men-sampling `public/brand/mark.png` lalu merakit logo phoenix dari bara menyala + kursor menyebarkan bara; reduced-motion → gambar logo statis; jumlah partikel auto-turun di mobile. Hero kini **gelap (bg-espresso)** + teks cream. **Navbar dibuat route-aware** (`usePathname()==="/" && !scrolled` → logo `onDark` + link cream; saat scroll → solid krem). Divider `bg-espresso → SectionDivider color="cream" curve` ditambah sebelum ServicesPreview. Dibuat lewat 3 opsi di `/hero-lab` (sudah dihapus); opsi B `AuroraStudio.tsx` (kartu produk parallax-tilt) & C `OrbitalPhoenix.tsx` (mark + orbit) **diparkir** di `src/components/hero/` untuk kemungkinan reuse. `HeroScene.tsx` lama dihapus.
